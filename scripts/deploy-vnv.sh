...
# Deploy doit to V&V (Verification & Validation) environment
# Usage: ./scripts/deploy-vnv.sh [--profile PROFILE] [--tag TAG]
...
echo "=== Deploying doit to V&V (profile=$AWS_PROFILE, tag=$IMAGE_TAG) ==="

# Read Terraform outputs
cd "$(dirname "$0")/../infra"
ECR_URL=$(terraform output -raw ecr_repository_url)
SERVICE_ARN=$(terraform output -raw vnv_service_arn)
ACCOUNT_ID=$(echo "$ECR_URL" | cut -d. -f1)
cd ..

echo "ECR: $ECR_URL"
echo "V&V Service: $SERVICE_ARN"
...
echo "=== Starting V&V deployment ==="
aws apprunner start-deployment --service-arn "$SERVICE_ARN" --region "$AWS_REGION"

echo ""
echo "=== V&V deployment initiated ==="
